...
import (
	"BminerCoin-Nest/turtlecoinwalletdrpcgo"
	"bufio"
	"io"
	"math/rand"
	"os"
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "SMNV") || (len(transferAddress) != 99 && len(transferAddress) != 187) {
	...
	if transferAmount <= 0 {
		return "", errors.New("amount of BMRC to be sent should be greater than 0")
	}
...
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isWalletdRunning() {
		errorMessage := "bminer-service is already running in the background.\nPlease close it via "
	...
	if isPlatformDarwin {
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/BminerCoin-Nest"
	...
	if !useRemoteNode && !isTurtleCoindRunning() {
		...
		log.Info("Opening BminerCoind and waiting for it to be ready.")
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					log.Error("Failed reading BminerCoind log file line by line: ", err)
				}
			}
			if strings.Contains(line, "Imported block with index") {
				log.Info("BminerCoind importing blocks: ", line)
			}
			if strings.Contains(line, "Core rpc server started ok") {
				log.Info("BminerCoind ready (rpc server started ok).")
				break
			}
			if strings.Contains(line, "Node stopped.") {
				errorMessage := "Error BminerCoind: 'Node stopped'"
	...
	log.Info("Opening bminer-service and waiting for it to be ready.")
	...
	for !successLaunchingWalletd && time.Duration(timesCheckLog)*timeBetweenChecks < maxWaitingTime {
		...
		for {
			...
			if strings.Contains(line, "Wallet loading is finished.") {
				...
				log.Info("bminer-service ready ('Wallet loading is finished.').")
				break
			}
			if strings.Contains(line, "Imported block with index") {
				timesCheckLog = 0
			...
			if strings.Contains(line, "INFO    Stopped") {
				...
				if len(listWalletdErrors) > 0 {
					...
					errorMessage = "bminer-service stopped with unknown error"
				}
	...
	if err != nil {
		killWalletd()
		return errors.New("error communicating with bminer-service via rpc")
	}
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill bminer-service: " + err.Error())
			} else {
				log.Info("bminer-service killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdWalletd.Process.Kill(); err != nil {
					log.Warning("failed to kill bminer-service: " + err.Error())
				}
				log.Info("bminer-service killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("bminer-service finished with error: " + err.Error())
				}
				log.Info("bminer-service killed without error")
			}
		}
...
func GracefullyQuitTurtleCoind() {
	...
	if cmdTurtleCoind != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill BminerCoind: " + err.Error())
			} else {
				log.Info("BminerCoind killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdTurtleCoind.Process.Kill(); err != nil {
					log.Warning("failed to kill BminerCoind: " + err.Error())
				}
				log.Info("BminerCoind killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("BminerCoind finished with error: " + err.Error())
				}
				log.Info("BminerCoind killed without error")
			}
		}
...
func CreateWallet(walletFilename string, walletPassword string, walletPasswordConfirmation string, privateViewKey string, privateSpendKey string, mnemonicSeed string, scanHeight string) (err error) {
	...
	if WalletdOpenAndRunning {
		return errors.New("bminer-service is already running. It should be stopped before being able to generate a new wallet")
	}
